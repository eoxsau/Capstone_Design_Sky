...
def analyze_frequency_distribution(df, type):
    ...
    csv_filename = f'frequency_counts_{type}_binned_{bin_info}.csv'
    ...
    plt.plot(range(len(frequency_counts)), frequency_counts.values, 
             linestyle='-', linewidth=1.5, color='skyblue')
    ...
    plt.tight_layout()
    plt.savefig(f'[{type}]analysis_binned_{bin_info}.png', dpi=300, bbox_inches='tight')
    plt.close()
