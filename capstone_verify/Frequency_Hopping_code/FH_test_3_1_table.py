...
def visualize_all(trials=10):
    ...
    tod = int(time.time()) & ((1 << 44) - 1)  # TOD 값 (44비트 제한)

    freq_results = [[] for _ in range(GROUP_COUNT)]  # 주파수 저장용
    group_order_results = [[] for _ in range(GROUP_COUNT)]  # 그룹 순서 저장용

    # --- 주파수 위치 데이터 저장 ---
    for group_id in range(GROUP_COUNT):
        for trial in range(trials):
            table = generate_group_frequencies(tod, trial, key)
            freq_results[group_id].append(table[group_id][0])

    # --- 그룹 순서 데이터 저장 ---
    for trial in range(trials):
        order = generate_group_order(tod, trial, key)
        for idx, gid in enumerate(order):
            group_order_results[gid].append(idx)

    return freq_results, group_order_results  # 결과 반환

if __name__ == "__main__":
    start_t = time.time()

    freqs, orders = visualize_all(trials=10)
