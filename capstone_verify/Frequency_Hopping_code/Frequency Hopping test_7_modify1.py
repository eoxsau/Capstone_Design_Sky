...
MIN_SPACING = 100e3  # 최소 이격 거리 = 10KHz
...
class Hopping_ver7:
    ...
    def generate_group_frequencies(self, tod):
        ...
        for gid in range(GROUP_COUNT + 1):
            ...
            norm1 = self.generate_complex_phase(z+0.001)
            rest_freqs.append(norm)
            index_freqs.append(norm1)
        sorted_indexes = np.argsort(index_freqs)
        # --- 0~1 사이의 정규화 값을 모두 더하면 1이 되도록 계산 ---
        error_sum = sum(rest_freqs)
        rest_freqs = [freq / error_sum for freq in rest_freqs]

        # --- 대역폭에 맞게 조정 ---
        rest_freqs = [freq * REST_BANDWIDTH for freq in rest_freqs]

        # --- 이전 이격으로부터 블럭 대역폭에 다음 이격을 더하면 다음 주파수 위치 ---
        group_freqs.append(rest_freqs[sorted_indexes[0]])
        for k in range(1, GROUP_COUNT):
            group_freqs.append(group_freqs[k - 1] + BLOCK_BANDWIDTH + rest_freqs[sorted_indexes[k]])
